...
export async function saveGuildConfig(
	...
	const now = new Date();
	const current = await guildRepository.findById(guildId);

	if (current) {
		await guildRepository.update(guildId, { ...data, lastUpdated: now });
		return;
	}

	await guildRepository.upsert(createDefaultGuild(guildId, { ...data, lastUpdated: now }, defaultAnnouncementMessage));
}
