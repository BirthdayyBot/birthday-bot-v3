...
export async function up(db: Kysely<unknown>): Promise<void> {
	...
	await db.schema.createIndex('index_birthday_birthday').on('birthday').column('birthday').ifNotExists().execute();
	await db.schema.createIndex('index_guild_id_birthday').on('birthday').column('guild_id').ifNotExists().execute();
	await db.schema.createIndex('index_user_id_birthday').on('birthday').column('user_id').ifNotExists().execute();
	await db.schema.createIndex('index_last_updated_guild').on('guild').column('last_updated').ifNotExists().execute();
	await db.schema.createIndex('index_user_birthday').on('user').column('username').ifNotExists().execute();
	...
	await db.schema.createIndex('index_guild_id_blacklist').on('blacklist').column('guild_id').ifNotExists().execute();
	await db.schema.createIndex('index_user_id_blacklist').on('blacklist').column('user_id').ifNotExists().execute();
